...
QEMUFLAGS := -m size=2G,slots=2,maxmem=4G -serial stdio
...
.PHONY: run-riscv
# -drive if=pflash,unit=0,format=raw,file=edk2-ovmf/ovmf-code-x86_64.fd,readonly=on
run-riscv: $(IMAGE_NAME).iso
	qemu-system-x86_64 \
		-M q35 \
		-drive if=pflash,unit=0,format=raw,file=riscv-firmware/fw_jump.elf,readonly=on
		-cdrom $(IMAGE_NAME).iso \
		-boot d \
		$(QEMUFLAGS)
...
$(IMAGE_NAME).iso: limine/limine kernel
	...
	BLAKE2BHASH=$$(b2sum iso_root/boot/kernel | cut -d ' ' -f 1)
	sed -i "s/hashtbr1/$$BLAKE2BHASH/g" limine.conf
	mkdir -p iso_root/boot/limine
	cp -v limine.conf limine/limine-bios.sys limine/limine-bios-cd.bin limine/limine-uefi-cd.bin iso_root/boot/limine/
	sed -i "s/$$BLAKE2BHASH/hashtbr1/g" limine.conf
	cp -v osbg.png iso_root/boot/
